...
double[] converted_XYZ = Utility.LBRtoXYZ(origin_LBR.ToArray());
...
double[] converted_LBR = Utility.XYZtoLBR(origin_XYZ.ToArray());
...
var test = vsop.GetPlanetPosition(VSOPBody.EARTH, VSOPVersion.VSOP87D, vTime);

Console.WriteLine(JsonSerializer.Serialize(test, new JsonSerializerOptions
{
    WriteIndented = true,
}));
...
void FormattedPrint(VSOPResult Result)
{
    ...
    Console.WriteLine("===============================================================");
    WriteColorLine("Reference: ", ConsoleColor.Green, $"\t\t{Enum.GetName(Result.FrameType)} | {Enum.GetName(Result.Epoch)}");
    ...
    {
        case Epoch.J2000:
            WriteColorLine("Epoch: ", ConsoleColor.Green, $"\t\t\tJ2000.0 (fixed)");
            break;

        case Epoch.OfDate:
            WriteColorLine("Epoch: ", ConsoleColor.Green, $"\t\t\tOf Date (precession applied)");
            break;
    }
